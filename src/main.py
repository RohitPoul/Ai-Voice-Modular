...
def get_audio_paths(audio_dir):
    ...
    for file in os.listdir(audio_dir):
        if file.endswith('_Instrumental.wav'):
            instrumentals_path = os.path.join(audio_dir, file)
            orig_audio_path = instrumentals_path.replace('_Instrumental', '')

        elif file.endswith('_Vocals_Main_DeReverb.wav'):
            main_vocals_dereverb_path = os.path.join(audio_dir, file)

        elif file.endswith('_Vocals_Backup.wav'):
            backup_vocals_path = os.path.join(audio_dir, file)

    return orig_audio_path, instrumentals_path, main_vocals_dereverb_path, backup_vocals_path
...
def preprocess_audio(audio_input, mdx_model_params, audio_id, is_webui, input_type, progress=None):
    keep_orig = False
    if input_type == 'yt':
        display_progress('[~] Downloading audio...', 0, is_webui, progress)
        audio_link = audio_input.split('&')[0]
        orig_audio_path = yt_download(audio_link)
    elif input_type == 'local':
        orig_audio_path = audio_input
        keep_orig = True
    else:
        orig_audio_path = None

    audio_output_dir = os.path.join(output_dir, audio_id)
    orig_audio_path = convert_to_stereo(orig_audio_path)

    display_progress('[~] Separating Vocals from Instrumental...', 0.1, is_webui, progress)
    vocals_path, instrumentals_path = run_mdx(mdx_model_params, audio_output_dir, os.path.join(mdxnet_models_dir, 'UVR-MDX-NET-Voc_FT.onnx'), orig_audio_path, denoise=True, keep_orig=keep_orig)

    display_progress('[~] Separating Main Vocals from Backup Vocals...', 0.2, is_webui, progress)
    backup_vocals_path, main_vocals_path = run_mdx(mdx_model_params, audio_output_dir, os.path.join(mdxnet_models_dir, 'UVR_MDXNET_KARA_2.onnx'), vocals_path, suffix='Backup', invert_suffix='Main', denoise=True)

    display_progress('[~] Applying DeReverb to Vocals...', 0.3, is_webui, progress)
    _, main_vocals_dereverb_path = run_mdx(mdx_model_params, audio_output_dir, os.path.join(mdxnet_models_dir, 'Reverb_HQ_By_FoxJoy.onnx'), main_vocals_path, invert_suffix='DeReverb', exclude_main=True, denoise=True)

    return orig_audio_path, vocals_path, instrumentals_path, main_vocals_path, backup_vocals_path, main_vocals_dereverb_path
...
def audio_cover_pipeline(audio_input, voice_model, pitch_change, keep_files,
    ...
    try:
        if not audio_input or not voice_model:
            raise_exception('Ensure that the audio input field and voice model field is filled.', is_webui)
        ...
        # if youtube url
        if urlparse(audio_input).scheme == 'https':
            input_type = 'yt'
            audio_id = get_youtube_video_id(audio_input)
            if audio_id is None:
        ...
        # local audio file
            ...
            if os.path.exists(audio_input):
                audio_id = get_hash(audio_input)
            else:
                error_msg = f'{audio_input} does not exist.'
                audio_id = None
                raise_exception(error_msg, is_webui)

        audio_dir = os.path.join(output_dir, audio_id)

        if not os.path.exists(audio_dir):
            os.makedirs(audio_dir)
            orig_audio_path, vocals_path, instrumentals_path, main_vocals_path, backup_vocals_path, main_vocals_dereverb_path = preprocess_audio(audio_input, mdx_model_params, audio_id, is_webui, input_type, progress)
            ...
            if any(path is None for path in paths) or keep_files:
                ...
                orig_audio_path, instrumentals_path, main_vocals_dereverb_path, backup_vocals_path = paths

        pitch_change = pitch_change * 12 + pitch_change_all
        ai_vocals_path = os.path.join(audio_dir, f'{os.path.splitext(os.path.basename(orig_audio_path))[0]}_{voice_model}_p{pitch_change}_i{index_rate}_fr{filter_radius}_rms{rms_mix_rate}_pro{protect}_{f0_method}{"" if f0_method != "mangio-crepe" else f"_{crepe_hop_length}"}.wav')
        ai_cover_path = os.path.join(audio_dir, f'{os.path.splitext(os.path.basename(orig_audio_path))[0]} ({voice_model} Ver).{output_format}')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Generate a AI cover audio in the audio_output/id directory.', add_help=True)
    parser.add_argument('-i', '--audio-input', type=str, required=True, help='Link to a YouTube video or the filepath to a local mp3/wav file to create an AI cover of')
    ...
    parser.add_argument('-k', '--keep-files', action=argparse.BooleanOptionalAction, help='Whether to keep all intermediate audio files generated in the audio_output/id directory, e.g. Isolated Vocals/Instrumentals')
    ...
    cover_path = audio_cover_pipeline(args.audio_input, rvc_dirname, args.pitch_change, args.keep_files,
