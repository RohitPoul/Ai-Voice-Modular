...
# Modified fairseq import section
import sys
import os

def load_hubert_from_checkpoint(checkpoint_path, device, is_half):
    """Custom implementation of checkpoint loading without direct fairseq dependency"""
    try:
        # Try using fairseq if available
        ...
        hubert = torch.load(checkpoint_path, map_location='cpu')
        if isinstance(hubert, dict) and 'model' in hubert:
            hubert = hubert['model']
    ...
    def device_config(self) -> tuple:
        if torch.cuda.is_available():
            ...
            if (
                ...
                preprocess_path = BASE_DIR / "src" / "trainset_preprocess_pipeline_print.py"
...
def load_hubert(device, is_half, model_path):
    return load_hubert_from_checkpoint(model_path, device, is_half)
