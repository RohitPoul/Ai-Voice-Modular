...
from main import audio_cover_pipeline
...
def load_public_models():
    models_table = []# No code was selected, so I will provide a general improvement to the code

# Add a function to handle exceptions and provide a more user-friendly error message
...
# Apply the handle_exception function to the download_online_model and upload_local_model functions
    download_online_model = handle_exception(download_online_model)
    upload_local_model = handle_exception(upload_local_model)

# Add a function to validate the model name and zip file
def validate_model_name_and_zip(model_name, zip_file):
    if not model_name:
        return gr.Error("Model name is required")
    if not zip_file:
        return gr.Error("Zip file is required")
    return None

# Apply the validate_model_name_and_zip function to the download_online_model and upload_local_model functions
    download_online_model = handle_exception(lambda model_zip_link, model_name: validate_model_name_and_zip(model_name, model_zip_link) or download_online_model(model_zip_link, model_name))
    upload_local_model = handle_exception(lambda zip_file, local_model_name: validate_model_name_and_zip(local_model_name, zip_file) or upload_local_model(zip_file, local_model_name))
...
if __name__ == '__main__':
    parser = ArgumentParser(description='Generate a AI generated audio in the audio_output/id directory.', add_help=True)
    ...
    with gr.Blocks(title='Agent Echo') as app:

        gr.Label('Agent Echo', show_label=False)
        ...
        with gr.Tab("Generate"):
            ...
            with gr.Accordion('Main Options'):
                with gr.Row():
                    with gr.Column():
                        rvc_model = gr.Dropdown(voice_models, label='Voice Models', info='Models folder "Agent Echo --> rvc_models". After new models are added into this folder, click the refresh button')
                        ref_btn = gr.Button('Refresh Models 🔁', variant='primary')

                    with gr.Column() as yt_link_col:
                        audio_input = gr.Text(label='Audio input', info='Link to a audio on YouTube or full path to a local file. For file upload, click the button below.')
                        show_file_upload_button = gr.Button('Upload file instead')

                    with gr.Column(visible=False) as file_upload_col:
                        local_file = gr.File(label='Audio file')
                        audio_input_file = gr.UploadButton('Upload 📂', file_types=['audio'], variant='primary')
                        show_yt_link_button = gr.Button('Paste YouTube link/Path to local file instead')
                        audio_input_file.upload(process_file_upload, inputs=[audio_input_file], outputs=[local_file, audio_input])
                    ...
                    show_file_upload_button.click(swap_visibility, outputs=[file_upload_col, yt_link_col, audio_input, local_file])
                    show_yt_link_button.click(swap_visibility, outputs=[yt_link_col, file_upload_col, audio_input, local_file])
            ...
            with gr.Accordion('Voice conversion options', open=False):
                ...
                keep_files = gr.Checkbox(label='Keep intermediate files', info='Keep all audio files generated in the audio_output/id directory, e.g. Isolated Vocals/Instrumentals. Leave unchecked to save space')
            ...
            with gr.Row():
                clear_btn = gr.ClearButton(value='Clear', components=[audio_input, rvc_model, keep_files, local_file])
                generate_btn = gr.Button("Generate", variant='primary')
                ai_cover = gr.Audio(label='AI Cover', show_share_button=False)

            ref_btn.click(update_models_list, None, outputs=rvc_model)
            is_webui = gr.Number(value=1, visible=False)
            generate_btn.click(audio_cover_pipeline,
                               inputs=[audio_input, rvc_model, pitch, keep_files, is_webui, main_gain, backup_gain,
...
app.launch(
    ...
    allowed_paths=["d:\\Agent Echo\\audio_output"]  # Allow access to the audio output directory
)